 ...
 * @since  1.6
 */
class Modern_toursModelCategories extends JModelList
{
	public $context;
	/**
	 * Constructor.
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__modern_tours_categories` AS a');
		$query->select('COUNT(asset.id) as tours_count, asset.id, asset.state as astate');
		$query->join('left', '#__modern_tours_assets AS asset ON asset.category=a.id');

		$appParams = JFactory::getApplication()->getParams('com_modern_tours');

		$categories = isset(MTHelper::getParams()->categories) ? MTHelper::getParams()->categories : $this->getState($this->context . 'items');
		$load_from = $appParams->get('load_from');

		if(!$categories && $load_from == 'categories')
		{
			$categories = $appParams->get('categories');
		}
		...
		if(isset($categories))
		{
			...
			$query->where('a.alias IN ("' . $categories . '")');
		}
